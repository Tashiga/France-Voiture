...
<article id="afficher_message">
    ...
    <div id="menu_personnes">
            <?php
            //s'il l'expediteur est un client
            if($_SESSION['client']['statut']==0) {
                $identifiant =  $_SESSION['client']['idClient'];
                echo ' <h3 class="titre_personnes">Clients</h3>';
                $result1 = $fonction_sql->executeRequete("select * from client where idClient != $identifiant");
                ...
                $result2 = $fonction_sql->executeRequete("select * from vendeur");
            ...
            //sinon l'expediteur est un vendeur
            else {
...
if($_POST){
    $statut = $_GET['statut'];
    $monStatut = $_SESSION['client']['statut'];
    $date = date('d-m-y');
    $temps = date("H:i:s");
    $messageFormulaire = $_POST['message'];
    $fonction_sql->executeRequete("INSERT into envoyer_un_message (idExpediteur, idDestinateur, statutExp, statutDest, message, dateEnvoye, tpsEnvoye)
     values($identifiant, $id_destinateur, '$monStatut' , '$statut', '$messageFormulaire', '$date', '$temps' )");
